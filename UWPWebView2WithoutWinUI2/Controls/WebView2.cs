...
namespace UWPWebView2WithoutWinUI2.Controls
{
    ...
    public partial class WebView2 : ContentControl
    {
        ...
        public async Task<string> ExecuteScriptAsync(string javascriptCode)
        {
            ...
            {
                throw new MethodAccessException(string.Format("ExecuteScriptAsync(): {0}", "WebView2NotPresent"));
            }
        }
        ...
        public void Reload()
        {
            ...
            else
            {
                ...
                {
                    throw new MethodAccessException(string.Format("ExecuteScriptAsync(): {0}", "WebView2NotPresentClosed"));
                }
                else
                {
                    throw new MethodAccessException(string.Format("ExecuteScriptAsync(): {0}", "WebView2NotPresent"));
                ...
                if (ShouldNavigate(newValueUri))
                {
                    if (!isClosed)
                    {
                        ...
                        if (coreWebView is not null)
                        {
                            ...
                            {
                                throw new Exception("WebView2Closed");
                            }
                        }
                    }
                    else
        ...
        private async Task CreateCoreObjectsAsync()
        {
            if (!isClosed)
            {
                ...
                else if (shouldShowMissingAnaheimWarning)
                {
                    ...
                    {
                        Text = "WarningSuitableWebView2NotFound"
                    };
                    warning.Inlines.Add(new LineBreak());
                    Run linkText = new()
                    {
                        Text = "DownloadWebView2Runtime"
                    };
